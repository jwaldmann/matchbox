...
topLevelDigrams :: (Ord sym) => Term var sym -> S.Set (Digram sym)
...
allDigrams :: (Ord sym) => Trees var sym -> S.Set (Digram sym)
allDigrams = S.unions . map digrams . terms

-- |Gets the set of all top level digrams from @Trees@
allTopLevelDigrams :: (Ord sym) => Trees var sym -> S.Set (Digram sym)
